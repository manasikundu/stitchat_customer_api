...
exports.FashionDesignerDetails = async (req, res) => {
  try {
    ...
    console.log("boutique id : ", btq_id)
    // const id = btq_id[0][0].boutique_id;
    if (btq_id && btq_id[0] && btq_id[0].length > 0) {
  ...
  try {
    ...
    var generateSlotResponse = async (slots, fashionDesignerStartTime, fashionDesignerEndTime, appointment_date) => {
      ...
      for (var slot of slots) {
        ...
        // var check_availability = status === 1 && isBooked ? true : false; 
        if(status==1 && !isBooked){
...
exports.fashionDesignerAppointmentDetails = async (req, res) => {
  try {
    ...
    if (result1) {
      ...
      var maskedNumber = result2 !== null && result2.mobile_number !== null ? await Service.maskMobileNumber(result2.mobile_number) : '';

      // var maskedNumber = result2.mobile_number !== null ? Service.maskMobileNumber(result2.mobile_number) : ''
      ...
      if (appointmentRatings) {
        ...
        rating.rate_id = appointmentRatings.id ? appointmentRatings.id : 0
